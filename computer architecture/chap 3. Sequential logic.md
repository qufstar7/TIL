...
### **1-1. 발진자**
주기적인 전기신호로 시간을 잰다.
- feedback(피드백, 되먹임) : 출력을 입력에 연결 (마이크와 스피커를 가까이)
    - 진동한다(발진한다. 발진자.)
...
---
## **3. 블록장치**
    디스크 드라이브 : 자화된 판에 비트 저장. 디스크 헤드가 손. 느리지만 대량 저장. 블록(섹터) 단위로 주소를 지정해 읽는다. 최근 디스크들은 방사상 영역으로 구분해 바깥쪽 트랙에 공간이 남는 낭비를 해결. 회전 지연 시간이 중요
  * 종류
    1. 하드 드라이브
    2. 플로피 디스크
    3. 자기 드럼 저장 장치   
...
---
## **6. 하드웨어와 소프트웨어 비교**
    소프트웨어는 설계에 들어가는 시간이라는 비용을 제외한 어떤 추가비용도 들지 않기 때문에 소프트웨어가 하드웨어보다 훨씬 더 만들기 쉽다는 점을 빼고는 차이가 거의 없다.  
