...
- [Spring Boot](#spring-boot)
	- [새 강의 등록하기](#새-강의-등록하기)
		- [강의 카테고리 가져오기의 작동](#강의-카테고리-가져오기의-작동)
...
* Model객체에 String "categories"에 categoires (카테고리목록 정보)가 저장됨.
  * HandelrAdapter에서 DB에서 가져온 categories data와 model객체에 저장된 String속성 data를 ModelAndView에 저장된다.
* DispatcherServlet는 ModelAndView를 
* InternalResourceViewResolver는 저장된 설정에 따라 해당하는 파일을 찾아서, 만약 있으면 JstlView를 DispatcherServlet에 반환한다.
* items Model객체에 저장해놓은 이름을 적어서 연결한다.
...
- 대표적인 트랜잭션 매니저
  ...
  - JpaTransactionManager - JPA 기술을 사용한 경주
